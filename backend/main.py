...
from app.engine import getSQLEngine
...
os.environ["MODEL_PROVIDER"] = "llama-api"
...
# initialized_engine = getSQLEngine()

# from app import loaddata2
...
@app.get("/QueryDB")
def getSQLResponse(query):
    from app.loaddata import getEngine
    # engine = getEngine()
    # engine.connect()
    response = qp.run(
        query=query
...
if __name__ == "__main__":
    ...
    uvicorn.run(app="main:app", loop="asyncio", host=app_host, port=app_port, reload=reload)
